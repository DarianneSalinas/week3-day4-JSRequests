...
const repeatMyParam = () => {
    ...
    axios.get('http://localhost:3000/repeat/{"testing-repeatMyParam"}')
    .then(response => {
        console.log(response.data)
    })
}

repeatParam = document.getElementById('repeat-button').addEventListener('click', repeatMyParam)
...
let repeatElementText = document.getElementById('repeat-text');
repeatElementText.textContent = response.data;
...
function queryTest() {
    //?key=value&value
    axios.get('http://localhost:3000/query-test/?vintagelover=all-things-vintage&antiques')
    .then((res => {
        console.log(res.data)
    })
}
